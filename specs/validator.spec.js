...
describe("request validator", function () {
  ...
  it("valid price for update orders", function () {
    ...
    expect(validateUpdate).withArgs([order], orders).to.not.throwException()
    delete order.price
    expect(validateUpdate).withArgs([order], orders).to.throwException()
    order.price = 0
    expect(validateUpdate).withArgs([order], orders).to.throwException(/should be greater than 0/)
    order.price = "i am not a number"
    expect(validateUpdate).withArgs([order], orders).to.throwException(/expecting number, found/)
    order.price = 12.123
    expect(validateCreate).withArgs([order], orders).to.throwException(/is invalid. price should be multiple of/)
  })
